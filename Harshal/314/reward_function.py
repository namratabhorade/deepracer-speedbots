...
# get absolute speed in continuous action space, value (1, 1.5, 2, 2.5, ... ,4)
...
def reward_function(params):
    ...
    if is_track_straight and (steering_angle == ps.steering_angle_1 == 0):
        reward += speed ** 2
        reward = round(reward, 2)
    ...
    if steering_angle != 0:
        ...
        if is_track_straight:
            reward *= (70 - abs(steering_angle)) / 70
            reward = round(abs(reward), 2)
