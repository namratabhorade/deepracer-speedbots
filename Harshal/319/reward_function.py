...
def reward_function(params):
    ...
    print("--------------------")
    reward = max((speed ** 2 + 10 * progress_to_steps_ratio) * (70 - direction_diff_angle) / 70, 2)
    ...
    if curve.is_track_straight:
        ...
        # Reward near to center on curves
        reward = (reward * 0.2) + (reward * 0.8 * near_center_per / 100)
        reward = round(reward, 2)
    ...
    if progress == 100:
        reward += progress_to_steps_ratio * 120
        reward = round(reward, 2)
