...
def reward_function(params):
    ...
    print("--------------------")
    reward = max((speed ** 2 + (1 + progress_to_steps_ratio) ** 4) * (100 - direction_diff_angle) / 100, 2)
    ...
    # reward for being on the opposite edge of curve
    elif curve.upcoming_curve_angle < 50 and curve.is_correct_side_of_curve:
        reward += (reward / 4) * (1 + abs((0.5 - near_center_per / 100)))
        reward = round(reward, 2)
    ...
    if progress < 100:
        ...
        reward += progress_to_steps_ratio * 200
        reward = round(reward, 2)
