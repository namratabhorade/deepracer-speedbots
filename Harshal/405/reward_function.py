...
def get_curve_details(params, length):
    ...
    curve.is_track_straight = (curve.upcoming_curve_angle <= 10)
...
def reward_function(params):
    ...
    print("--------------------")
    initial_reward = (speed + 6) * (
            (80 - direction_diff_angle) / 80) * progress_to_steps_ratio
    ...
    if not is_left_of_center or curve.is_correct_side_of_curve:
        reward += reward * 0.4
    ...
    if curve.is_track_straight:
        ...
        if curve.upcoming_curve_angle + direction_diff_angle > 60:
            reward *= ((100 - direction_diff_angle) / 100) / speed
            reward = round(abs(reward), 2)
            print("2b1) reward:", reward)

    if progress < 100:
        reward += reward * (progress / 10) * progress_to_steps_ratio
        reward = round(abs(reward), 2)
        print("3a) reward:", reward)
    else:
        reward += ((progress_to_steps_ratio ** 5) * 2000) + (reward * (progress / 10) * progress_to_steps_ratio)
    ...
    reward = round(max(min(reward, 96000), 0.01), 2)
    print("--------------------")
