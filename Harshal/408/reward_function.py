...
def get_curve_details(params, length):
    ...
    curve.is_track_straight = (curve.upcoming_curve_angle <= 20)
...
def reward_function(params):
    ...
    print("--------------------")
    initial_reward = 1 + (speed * progress / 20 * progress_to_steps_ratio)
    ...
    if not is_left_of_center or curve.is_correct_side_of_curve:
        reward += reward * 0.25 * near_center_per / 100
    ...
    if curve.is_track_straight:
        ...
        if curve.upcoming_curve_angle + direction_diff_angle > 75:
